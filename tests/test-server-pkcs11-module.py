...
if __name__ == "__main__":
  ...
  try:
    ...
    p = Popen('find /usr -name libsofthsm2.so | head -n1', shell=True, stdout=PIPE)
    p.wait()

    if p.returncode != 0:
      raise Exception('unable to find libsofthsm2.so for pkcs11 testing')

    modulePath = str(p.stdout.read()).strip()
